...
@endsection
@section('bagan')
<form action="/buyFracture/print" method="post">
    @csrf
    <a href="/buyFracture/{{ $nomorRegis->id }}/print" class="btn btn-primary">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row mb-4">
                ...
                <div class="col-sm-6 ">
                    <h5 class="mb-3">To:</h5>
                    <h3 class="text-dark mb-1">{{ $supplier->nama }}</h3>
                    <div>{{ $supplier->alamat }}</div>
                    <div>{{ $supplier->noTelp }}</div>
                </div>
            </div>
                    ...
                    <tbody>
                        @foreach ($buyFractures as $buyFracture)    
                            ...
                            <tr class="table table-primary">
                                ...
                                <td class="right">{{ $buyFracture->harga }}</td>
                                <td class="center">{{ $buyFracture->qty }}</td>
                                <td class="right">{{ $buyFracture->total }}</td> 
                            </tr>
                            @else
                            <tr>
                                <td class="center">{{ $loop->iteration }}</td>
